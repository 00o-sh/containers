...
| transmission | ✅ **Done.** C/cmake source build (Wolfi ships nothing; apps/ used Alpine *edge* apks) with vendored submodules, bundled web UI built via npm (`ENABLE_WEB=ON`), unrar built from rarlab source + Wolfi 7zip for torrent-done tooling parity, and a repo-local Go shim (`src/`) replacing the bash+minijinja entrypoint. Empirical finding: the apps/ template rendered **invalid JSON** (unquoted strings, dangling values) so env-driven settings never actually loaded — the shim fixes this with typed rendering; unset no-default keys are omitted. Tests: default-boot 403 parity + env-templated boot serving the web UI at 200 (proves rendered config loads). Shell-free caveat: torrent-done scripts must be direct executables. | HTTP smoke `:9091` |
